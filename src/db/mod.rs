...
fn migrate_from_bincode(conn: &mut Connection, old_path: &Path) -> Result<()> {
    ...
    for dir in dirs {
        ...
        tx.execute(
            ...
            params![&path_s, dir.rank, dir.last_accessed as i64],
        )?;
    }
    tx.commit()?;
...
impl Database {
    ...
    pub fn open_dir(data_dir: impl AsRef<Path>) -> Result<Self> {
        ...
        conn.execute_batch(
            ...
            CREATE INDEX IF NOT EXISTS idx_dirs_last_accessed ON dirs(last_accessed);",
        )?;
    ...
    pub fn add(&mut self, path: impl AsRef<str> + Into<String>, by: Rank, now: Epoch) {
        ...
        let existing: Option<(f64, u64)> = tx
            .query_row(
                ...
                params![&path_s],
                |row| Ok((row.get(0)?, row.get::<_, i64>(1)? as Epoch)),
            )
            .optional()
            .unwrap_or(None);
        ...
        match existing {
            ...
            None => {
                let _ = tx.execute(
                    ...
                    params![&path_s, by.max(0.0), now as i64],
                );
            }
        }
        ...
        match existing {
            Some((rank, _)) => {
                ...
                let _ = tx.execute(
                    ...
                    params![new_rank, now as i64, &path_s],
                );
            }
            None => {
                let _ = tx.execute(
    ...
    pub fn dirs(&self) -> Vec<Dir<'static>> {
        ...
        let rows = stmt.query_map([], |row| {
            Ok(Dir {
                ...
                last_accessed: row.get::<_, i64>(2)? as Epoch,
            })
        });
...
mod tests {
    ...
    fn migrate_from_bincode() {
        ...
        let dirs = vec![Dir { path: "/foo".into(), rank: 1.0, last_accessed: 12345 }];
